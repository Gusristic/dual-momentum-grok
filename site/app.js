(function(){
  ...
  Promise.all(['b0.txt','b1.txt','b2.txt','b3.txt'].map(function(s){
    return fetch(s,{cache:'no-store'}).then(function(r){ if(!r.ok) throw new Error(s); return r.text(); });
  })).then(function(parts){
    return inflate(parts.join(''));
  }).then(function(code){
